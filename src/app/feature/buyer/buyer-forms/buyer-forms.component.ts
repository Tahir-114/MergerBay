...
@Component({
  selector: 'app-buyer-forms',
  templateUrl: './buyer-forms.component.html',
  styleUrls: ['./buyer-forms.component.css']
})
export class BuyerFormsComponent implements OnInit {

  selectedCard: string[] = [];
  selectedSector: string[] = [];
  selectedOwnershipPref: string[] = [];
  selectedRevenuePreference: string[] = [];
  transactionRole: number = 0;

  cards = [
    { id: 1, name: 'Cash Deal' },
    { id: 2, name: 'Share Swap' },
    { id: 3, name: 'Cash and Share Swap' },
    { id: 4, name: 'Cash (Default)' },
  ];
  sectors_interested = [
    { id: 1, name: 'Energy' },
    { id: 2, name: 'Materials' },
  ...
  tab3 = false;
  tab4 = false;
  constructor(private config: NgSelectConfig, private url: LocationStrategy) {

  }

  ngOnInit(): void {
    if(this.url.path() === '/buyer/buyer-form/buy-out'){
      this.ind_page = true;
      this.tab1 = true;
    }
    if(this.url.path() === '/buyer/buyer-form/investment'){
      this.ind_page = true;
      this.tab2 = true;
    }
    if(this.url.path() === '/buyer/buyer-form/mergers-acquisitions'){
      this.ind_page = true;
      this.tab3 = true;
    }
    if(this.url.path() === '/buyer/buyer-form/commercial-property'){
      this.ind_page = true;
      this.tab4 = true;
    }
  }
  ...
  load_sectors(event: any) {
    console.log(this.selectedSector?.length);
  }
  ...
  c_bo_inst_country(event: string, selector: string) {
    if (event != '' && typeof(event) != "undefined") {
      event = event.toLowerCase();
      this.bo_inst_country = `flag_holder flag flag-${event}`;
    } else {
